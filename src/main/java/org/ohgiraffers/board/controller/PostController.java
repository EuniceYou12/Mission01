 ...
 * Controller 주로 화면 View 를 반환 하기위해 사용된다.
 * 하지만 종종 Controller를 쓰면서도 데이터를 반환 해야할 때가 있는데, 이럴 때 사용하는 것이 @ResponseBody
 * Rest 란?
 * Representational AState Transfer 의 약자
 * 자원을 이름으로 구분하여 자원의 상태를 주고 받는 것을 의미한다.
 * Rest는 기본적으로 웹의 기본 기술과 Http 프로토콜을 그대로 사용하기 때문에,
...
public class PostController {
    ...
    public ResponseEntity<CreatePostResponse> postCreate(@RequestBody CreatePostRequest request) {
        ...
        CreatePostResponse response = postService.creatPost(request);
